...
export async function getAllPosts({
  ...
  try {
    ...
    const collectionData = normalizeCollection(rawCollection)
    const schema = normalizeSchema(collectionData?.schema)
    ...
    for (let i = 0; i < resolvedPageIds.length; i++) {
      ...
      const blockValue = normalizePageBlock(blockItem) || blockItem?.value
