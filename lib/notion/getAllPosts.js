...
export async function getAllPosts({
  ...
  try {
    ...
    // FIX: Schema'yı doğru şekilde al
    const collectionId = Object.keys(response.collection || {})[0]
    let collectionData = collectionId ? response.collection[collectionId]?.value : null
    let schema = collectionData?.schema

    // DÜZELTME: Loglarda görülen iç içe geçmiş 'value' yapısını kontrol et
    if (!schema && collectionData?.value?.schema) {
      console.warn('Schema nested in value property, correcting structure...')
