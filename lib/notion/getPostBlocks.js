...
export async function getPostBlocks(id) {
  ...
  try {
    ...
    // İlk normalize işlemi (nested .value.value unwrap)
    normalizeRecordMapValues(pageBlock)

    pageBlock.collection_query = pageBlock.collection_query || {}

    // 1. pageBlock.collection_view altındaki tüm görünümleri tespit et ve çek
    for (const [viewId, viewItem] of Object.entries(pageBlock.collection_view || {})) {
      const viewVal = viewItem?.value || viewItem
      const parentBlock =
        pageBlock.block?.[viewVal?.parent_id]?.value ||
        pageBlock.block?.[viewVal?.parent_id]

      const collectionId =
        viewVal?.format?.collection_pointer?.id ||
        parentBlock?.collection_id ||
        parentBlock?.format?.collection_pointer?.id ||
        (parentBlock ? getBlockCollectionId(parentBlock, pageBlock) : null)

      if (collectionId && viewId && !pageBlock.collection_query[collectionId]?.[viewId]) {
        try {
          ...
          pageBlock.collection_query[collectionId][viewId] =
            collectionData?.result?.reducerResults ||
            collectionData?.result ||
            {}
        } catch (err) {
          console.warn(
            `Failed to fetch collection data for collection ${collectionId}, view ${viewId}:`,
            err.message
          )
        }
      }
    ...
    // 2. pageBlock.block içindeki collection_view bloklarını da kontrol et
    for (const blockItem of Object.values(pageBlock.block || {})) {
      const blockVal = blockItem?.value || blockItem
      if (
        blockVal &&
        (blockVal.type === 'collection_view' || blockVal.type === 'collection_view_page')
      ) {
        const collectionId = getBlockCollectionId(blockVal, pageBlock) || blockVal.collection_id
        ...
        for (const viewId of viewIds) {
          if (
            ...
            try {
              const viewItem = pageBlock.collection_view?.[viewId]
              const viewVal = viewItem?.value || viewItem
