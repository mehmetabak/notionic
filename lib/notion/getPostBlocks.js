...
export async function getPostBlocks(id) {
  ...
  try {
    ...
    // 1. pageBlock.collection_view altındaki tüm görünümleri çek
    for (const [viewId, viewItem] of Object.entries(pageBlock.collection_view || {})) {
      const viewVal = viewItem?.value || viewItem

      // view'ın bağlı olduğu collection ID'yi bul
      let collectionId = viewVal?.parent_id
      if (!collectionId || !pageBlock.collection?.[collectionId]) {
        for (const colId of Object.keys(pageBlock.collection || {})) {
          collectionId = colId
          break
        }
      }
