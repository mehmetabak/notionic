...
function getAllContentBlockIds(recordMap) {
  ...
  for (const blockItem of Object.values(recordMap.block)) {
    ...
    // 2. Synced blocks (transclusion reference pointer)
    if (blockVal.format?.transclusion_reference_pointer?.id) {
      ids.add(blockVal.format.transclusion_reference_pointer.id)
    }

    // 3. Page alias / link pointer
    if (blockVal.format?.alias_pointer?.id) {
      ids.add(blockVal.format.alias_pointer.id)
    }

    // 4. Linked page mentions in properties (e.g. [["‣", [["p", "id"]]]])
    if (blockVal.properties) {
      for (const prop of Object.values(blockVal.properties)) {
        if (Array.isArray(prop)) {
          for (const item of prop) {
            if (Array.isArray(item)) {
              for (const decoration of item) {
                if (Array.isArray(decoration)) {
                  for (const sub of decoration) {
                    if (Array.isArray(sub) && (sub[0] === 'p' || sub[0] === 'b') && sub[1]) {
                      ids.add(sub[1])
                    }
                  }
                }
